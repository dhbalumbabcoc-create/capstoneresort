...
// ── Group by reference_number, guest_email, and booking_created_at ───────────
// so that sibling bookings collapse into ONE row, while preventing unrelated
// payments/bookings with the same reference number from collapsing.
// ─────────────────────────────────────────────────────────────────────────────
$groups      = []; // tx_key => representative payment row
$group_bids  = []; // tx_key => [booking_id, ...]  (unique, for total_due)
$group_paid  = []; // tx_key => sum of amount_paid (deduplicated by payment id)
$group_pids  = []; // tx_key => [payment_id, ...] seen
...
foreach ($payments_raw as $p) {
    ...
    $cat    = $p['booking_created_at'];

    // Payments with no ref number or standard walk-in payments are solo groups
    if ($ref !== '' && $ref !== 'WALKIN-CASH') {
        $tx_key = $ref . '_' . $email . '_' . $cat;
    } else {
        $tx_key = '__solo__' . $p['id'];
    }
    ...
    // Use lowest booking_id as the primary display booking
    ...
    // Sum total_price only once per distinct booking_id
    if (!in_array($bid, $group_bids[$tx_key])) {
        $group_bids[$tx_key][] = $bid;
        $group_due[$tx_key]   += floatval($p['total_price']);
    }

    // Sum amount_paid only once per distinct payment id
...
// Build final flat list — one row per unique transaction group
$payments = [];
foreach ($groups as $tx_key => $row) {
    $row['tx_total_due']   = $group_due[$tx_key];
    $row['tx_total_paid']  = $group_paid[$tx_key];
    $row['tx_remaining']   = $group_due[$tx_key] - $group_paid[$tx_key];
    $row['tx_booking_ids'] = $group_bids[$tx_key]; // all sibling booking IDs
    $payments[] = $row;
}
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-6 col-lg-3">
                    <div class="kpi-card">
                        ...
                        <div><div class="kpi-num">₱<?= number_format($total_collected, 0) ?></div><div class="kpi-lbl">Verified Collections</div></div>
                    </div>
                </div>
                <div class="col-6 col-lg-3">
                    <div class="kpi-card">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tr>
                            ...
                            <td><span class="ref-code"><?= htmlspecialchars($p['reference_number'] ?? '&mdash;') ?></span></td>
                        ...
                        <?php endforeach; else: ?>
                        <tr><td colspan="10" class="text-center text-muted py-4"><i class="fas fa-inbox me-2"></i>No online transactions found.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-outline-primary flex-fill" id="btnGcash" onclick="setMethod('online')">
                                <i class="fas fa-mobile-alt me-1"></i> GCash
                            </button>
                            <button type="button" class="btn btn-outline-secondary flex-fill" id="btnWalkin" onclick="setMethod('walkin')">
...
function setMethod(m) {
    ...
    const lbl  = document.querySelector('#refGroup label');
    if (m === 'online') {
        btnG.className = 'btn btn-primary flex-fill';
        btnW.className = 'btn btn-outline-secondary flex-fill';
        refInput.setAttribute('required','required');
        lbl.textContent = 'GCash Reference Number';
        refInput.placeholder = 'e.g. 1234567890123';
    } else {
        btnW.className = 'btn btn-secondary flex-fill';
        btnG.className = 'btn btn-outline-primary flex-fill';
        refInput.removeAttribute('required');
        lbl.textContent = 'Receipt / Note (Optional)';
        refInput.placeholder = 'e.g. Cash paid at front desk';
    }
}
