...
if ($booking_id <= 0) { header("Location: " . BASE_URL . "admin/walkin_booking.php"); exit(); }
...
if (!$booking) { header("Location: " . BASE_URL . "admin/walkin_booking.php"); exit(); }
...
$bk_trans = 'none';
$trans_cost = 0;
if (!empty($booking['notes']) && preg_match('/Transport:\s*([a-z0-9_\-]+)/i', $booking['notes'], $m)) {
    $bk_trans = strtolower($m[1]);
    $trans_guests = max(0, intval($booking['num_guests']) - intval($booking['num_below5'] ?? 0));
    if ($bk_trans === 'tignapoloan') $trans_cost = $trans_guests * 50;
    elseif ($bk_trans === 'cdo') $trans_cost = $trans_guests * 250;
    elseif ($bk_trans === 'private') $trans_cost = 3500;
}
...
// Send receipt email if guest has email and not yet sent
            ...
            <div class="receipt-wrap">
                ...
                <div class="receipt-card">
                    ...
                    <div class="receipt-body">
                        ...
                        <?php endif; ?>
                        <?php if ($trans_cost > 0): 
                            $trans_lbl = $bk_trans === 'tignapoloan' ? 'Tignapoloan Crossing' : ($bk_trans === 'cdo' ? 'Cagayan De Oro' : 'Private Vehicle Rental');
                        ?>
                        <div class="detail-row">
                            <span class="detail-label">Transportation (<?= $trans_lbl ?>)</span>
                            <span class="detail-value">&#8369;<?= number_format($trans_cost, 2) ?></span>
                        </div>
                        <?php endif; ?>
                        <?php if (!empty($addons)): ?>
