...
if ($step === 'password') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_password'])) {
        ...
        if (strlen($new_pass) < 8) {
            ...
            $stmt = $conn->prepare("UPDATE users SET password=?, must_change_password=0, email_verified=1, status='active', setup_otp=NULL, setup_otp_expires=NULL WHERE id=?");
            $stmt->bind_param('si', $hashed, $user_id);
            $stmt->execute();
            $stmt->close();

            // Clear setup & login session vars so the user logs in fresh with new password
            unset($_SESSION['user_id'], $_SESSION['user_role'], $_SESSION['user_name'], $_SESSION['user_first_name'], $_SESSION['last_activity'], $_SESSION['username'], $_SESSION['setup_step'], $_SESSION['otp_sent'], $_SESSION['otp_verified']);
            set_success_message('Account activated successfully! Please log in with your new password.');
            $success = 'Account activated successfully! Redirecting to login page...';
        }
    }
}
?>
...
body{font-family:'Inter',sans-serif;min-height:100vh;background:url('<?php echo $base_img; ?>bolao.jpg') center/cover no-repeat fixed;display:flex;align-items:center;justify-content:center;padding:clamp(12px,2vh,24px) 16px;position:relative;}
body::before{content:'';position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:0;}
.setup-card{position:relative;z-index:1;background:#fff;border-radius:18px;padding:clamp(20px,3vh,28px) clamp(20px,3vw,30px) clamp(16px,2vh,22px);width:100%;max-width:400px;box-shadow:0 8px 48px rgba(0,0,0,.3);max-height:calc(100vh - 24px);overflow-y:auto;scrollbar-width:thin;scrollbar-color:rgba(0,0,0,.15) transparent;}
.setup-card::-webkit-scrollbar{width:4px;}
.setup-card::-webkit-scrollbar-thumb{background:rgba(0,0,0,.15);border-radius:4px;}
.setup-logo{display:flex;align-items:center;gap:10px;margin-bottom:18px;}
.setup-logo img{width:38px;height:38px;border-radius:10px;object-fit:cover;border:2px solid #e8f5e9;}
.setup-logo strong{display:block;font-size:.86rem;font-weight:800;color:#111;}
.setup-logo span{font-size:.72rem;color:#aaa;}
.step-indicator{display:flex;align-items:center;gap:8px;margin-bottom:18px;}
.step-dot{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.76rem;font-weight:700;flex-shrink:0;}
...
.setup-heading{margin-bottom:16px;}
.setup-heading h2{font-size:1.35rem;font-weight:800;color:#111;margin-bottom:4px;}
.setup-heading p{font-size:.84rem;color:#888;line-height:1.5;}
.alert-error{background:#fdecea;color:#c62828;border:1.5px solid #f5c6cb;border-radius:10px;padding:9px 12px;font-size:.82rem;display:flex;align-items:center;gap:8px;margin-bottom:14px;animation:shake .35s ease;}
.alert-success{background:#e8f5e9;color:#1B7D3A;border:1.5px solid #c8e6c9;border-radius:10px;padding:9px 12px;font-size:.82rem;display:flex;align-items:center;gap:8px;margin-bottom:14px;}
...
<div class="setup-card">
    ...
    <script>setTimeout(()=>location.href='<?= BASE_URL ?>login.php',1500);</script>
    <?php endif; ?>
