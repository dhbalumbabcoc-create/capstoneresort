...
switch ($filter_period) {
    case 'today':
        $date_where = "AND DATE(b.created_at) = '$today'";
        break;
    case 'week':
        $date_where = "AND DATE(b.created_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
        break;
    case 'month':
        $date_where = "AND MONTH(b.created_at) = MONTH(CURDATE()) AND YEAR(b.created_at) = YEAR(CURDATE())";
        break;
    default:
        $date_where = '';
}

// ── KPI: pull from bookings table (confirmed = paid) ─────────────────────────
$confirmed_statuses = "'confirmed','completed'";

// Total revenue (confirmed bookings)
$rev_q = $conn->query("SELECT SUM(total_price) AS total, COUNT(*) AS cnt FROM bookings b WHERE status IN ($confirmed_statuses) $date_where");
$rev_row = $rev_q->fetch_assoc();
...
$pending_rev = $conn->query("SELECT SUM(total_price) AS t FROM bookings b WHERE status='pending' $date_where")->fetch_assoc()['t'] ?? 0;

// By booking type
$online_q  = $conn->query("SELECT SUM(total_price) AS total, COUNT(*) AS cnt FROM bookings b WHERE booking_type='online' AND status IN ($confirmed_statuses) $date_where");
$walkin_q  = $conn->query("SELECT SUM(total_price) AS total, COUNT(*) AS cnt FROM bookings b WHERE booking_type IN ('walkin','walk_in','walk-in') AND status IN ($confirmed_statuses) $date_where");
$online_row = $online_q->fetch_assoc();
$walkin_row = $walkin_q->fetch_assoc();
$online_total = floatval($online_row['total'] ?? 0);
$online_cnt   = intval($online_row['cnt']   ?? 0);
$walkin_total = floatval($walkin_row['total'] ?? 0);
$walkin_cnt   = intval($walkin_row['cnt']   ?? 0);
...
for ($i = 5; $i >= 0; $i--) {
    ...
    $monthly[] = [
        ...
        'rev'   => floatval($row['rev'] ?? 0),
        'cnt'   => intval($row['cnt']  ?? 0),
    ];
}

// Recent confirmed bookings (last 15)
...
<head>
    ...
    <title>Payment Summary - Resort Management</title>
    ...
    <div class="content">
        ...
        <div class="dash-body">
            ...
            <div class="d-flex gap-2 mb-4 flex-wrap">
                ...
                <a href="?period=<?= $k ?>" style="padding:7px 18px;border-radius:20px;font-size:.82rem;font-weight:700;text-decoration:none;border:1.5px solid <?= $filter_period===$k?'transparent':'#e0e0e0' ?>;background:<?= $filter_period===$k?'linear-gradient(135deg,#1B7D3A,#27A457)':'#fff' ?>;color:<?= $filter_period===$k?'#fff':'#555' ?>;">
                    <?= $lbl ?>
                </a>
                <?php endforeach; ?>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-6 col-lg-3">
                    <div class="kpi-card">
                        ...
                        <div>
                            <div class="kpi-num" data-count="<?= intval($total_revenue) ?>" data-prefix="&#8369;">&#8369;<?= number_format($total_revenue,2) ?></div>
                ...
                <div class="col-6 col-lg-3">
                    <div class="kpi-card">
                        ...
                        <div>
                            <div class="kpi-num" data-count="<?= $total_confirmed ?>"><?= $total_confirmed ?></div>
                ...
                <div class="col-6 col-lg-3">
                    <div class="kpi-card">
                        ...
                        <div>
                            <div class="kpi-num" data-count="<?= $pending_cnt ?>"><?= $pending_cnt ?></div>
                ...
                <div class="col-6 col-lg-3">
                    <div class="kpi-card">
                        ...
                        <div>
                            <div class="kpi-num" data-count="<?= intval($today_rev) ?>" data-prefix="&#8369;">&#8369;<?= number_format($today_rev,2) ?></div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-lg-6">
                    ...
                    <div class="table-card d-flex align-items-center gap-4">
                        <div style="width:64px;height:64px;border-radius:16px;background:#e3f2fd;display:flex;align-items:center;justify-content:center;font-size:1.6rem;color:#1565c0;flex-shrink:0;">
                            <i class="fas fa-globe"></i>
                        </div>
                        <div>
                            <div style="font-size:2rem;font-weight:900;color:#1565c0;">&#8369;<?= number_format($online_total,2) ?></div>
                            <div style="font-size:.85rem;color:#888;"><?= $online_cnt ?> confirmed booking<?= $online_cnt!=1?'s':'' ?></div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                    ...
                    <div class="table-card d-flex align-items-center gap-4">
                        <div style="width:64px;height:64px;border-radius:16px;background:#e8f5e9;display:flex;align-items:center;justify-content:center;font-size:1.6rem;color:#1B7D3A;flex-shrink:0;">
                            <i class="fas fa-walking"></i>
                        </div>
                        <div>
                            <div style="font-size:2rem;font-weight:900;color:#1B7D3A;">&#8369;<?= number_format($walkin_total,2) ?></div>
                            <div style="font-size:.85rem;color:#888;"><?= $walkin_cnt ?> confirmed booking<?= $walkin_cnt!=1?'s':'' ?></div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="table-card">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                $rows = [
                                    ['Online',  $online_cnt, $online_total, '#1565c0', 'pill-blue'],
                                    ['Walk-in', $walkin_cnt, $walkin_total, '#1B7D3A', 'pill-green'],
                                ];
                                foreach ($rows as $r):
                                    ...
                                    <td>
                                        <div style="display:flex;align-items:center;gap:8px;">
                                            <div style="flex:1;height:6px;background:#f0f0f0;border-radius:3px;overflow:hidden;">
                                                <div style="width:<?= $pct ?>%;height:100%;background:<?= $r[3] ?>;border-radius:3px;"></div>
                                            </div>
                                            <span style="font-size:.78rem;color:#888;white-space:nowrap;"><?= $pct ?>%</span>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <tr style="background:#f8fffe;">
                                    ...
                                    <td><strong><?= $online_cnt + $walkin_cnt ?></strong></td>
                                    <td><strong style="color:#1B7D3A;">&#8369;<?= number_format($grand,2) ?></strong></td>
            ...
            <div class="section-hdr">
                <h5><i class="fas fa-list me-2" style="color:#1B7D3A;"></i>Recent Confirmed Bookings</h5>
                <p>Latest 15 confirmed bookings<?= $filter_period!=='all'?' for selected period':'' ?></p>
            </div>
            <div class="table-card">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.kpi-num[data-count]').forEach((el,i) => {
        ...
        setTimeout(() => {
            ...
            const u = (n) => {
                ...
                el.textContent = pfx + (Number.isInteger(t) ? Math.round(v).toLocaleString() : v.toFixed(2));
    ...
    new Chart(document.getElementById('typeChart').getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{
                data: [<?= $online_total ?>, <?= $walkin_total ?>],
                backgroundColor: ['#e3f2fd', '#e8f5e9'],
                borderColor:     ['#1565c0', '#1B7D3A'],
                borderWidth: 2,
                hoverOffset: 8
            }]
        },
    ...
    new Chart(ctx2, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Revenue',
                data: monthRev,
                backgroundColor: monthRev.map((v,i) => {
                    const max = Math.max(...monthRev);
        ...
        options: {
            ...
            scales: {
                y: { beginAtZero: true, ticks: { callback: v => '₱'+(v>=1000?(v/1000).toFixed(0)+'k':v), font:{size:11}, color:'#888' }, grid:{color:'rgba(0,0,0,.04)'}, border:{display:false} },
